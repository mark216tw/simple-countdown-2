...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val uiState = combine(
        ...
        initialValue = AppUiState(timer = timer.value),
    )
    ...
    fun startTimer(preset: TimerPreset) = timerAction("無法啟動背景計時服務。") {
        timerRepository.start(preset, uiState.value.settings)
    }

    fun restartTimer() {
        val snapshot = timer.value
    ...
    fun pauseTimer() = timerAction { timerRepository.pause() }

    fun resumeTimer() = timerAction { timerRepository.resume() }

    fun addTime(seconds: Long) = timerAction { timerRepository.addTime(seconds) }

    fun resetTimer() = timerAction { timerRepository.reset() }

    fun stopTimer() = timerAction { timerRepository.stop() }

    fun dismissAlarm() = timerAction { timerRepository.dismissAlarm() }
    ...
    private fun refreshTimer() {
        timer.value = timerRepository.state()
    }
}
